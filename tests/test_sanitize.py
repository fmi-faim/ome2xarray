...
def test_sanitize_pixels_basic():
    ...
    image = Image(id="Image:0", name="test_image", pixels=pixels)
...
def test_sanitize_pixels_removes_stage_label_from_image_name():
    ...
    assert (
        sanitized.tiff_data_blocks[0].uuid.file_name
        == "20260109_SWI_3501_eft_col1_w1DAPI_s20.ome.tif"
    )
...
def test_companion_file_filenames_with_and_without_sanitize_sg1(filename):
    ...
    companion_file_path = (
        Path(__file__).parent / "resources" / "20250910_VV7-0-0-6-ScanSlide" / filename
    )
    ...
    original_filenames = {
        block.uuid.file_name for block in original_image.pixels.tiff_data_blocks
    }
    ...
    sanitized_filenames = {
        block.uuid.file_name for block in sanitized_image.pixels.tiff_data_blocks
    }
...
def test_companion_file_filenames_without_include_sg():
    ...
    for san_fn in sanitized_filenames:
        assert "_sg" not in san_fn, (
            f"Sanitized filename should not contain _sg: {san_fn}"
        )
...
def test_sanitize_pixels_channel_list_none_uses_default():
    ...
    assert len(sanitized.tiff_data_blocks) == 2
    assert (
        sanitized.tiff_data_blocks[0].uuid.file_name == "test_image_w1OriginalA.ome.tif"
    )
    assert (
        sanitized.tiff_data_blocks[1].uuid.file_name == "test_image_w2OriginalB.ome.tif"
    )
